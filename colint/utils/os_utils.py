...
def get_valid_files(path: str) -> list[str]:
    """
    Retrieves a list of valid file paths in the specified directory,
    excluding files in the '.git' directory and ignored files according to Git.
    ...
    files = [str(f) for f in Path(path).rglob("*") if f.is_file() and ".git" not in f.parts]
